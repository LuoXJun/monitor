...
export const generateRoute = (routes: AppCustomRouteRecordRaw[]): AppRouteRecordRaw[] => {
  ...
  for (const route of routes) {
    ...
    // 其实就是判断component是否包含?，然后解析参数，放到meta.query中
    ...
    //殊逻辑：如果后端配置的 MenuDO.path 包含 ?，则表示需要传递参数
    //此时，我们需要解析参数，并且将参数放到 meta.query 中
...
//根据父级路由生成子路由
export const generateMultiLevelRoutes = (routes: AppRouteRecordRaw[],parentName:string) => {
  //console.log(routes)
  //保存父级路径
  let parentPath =''
  //获取指定父级路径的子路由
  const rt: AppRouteRecordRaw[] = routes.flatMap(route => {
      if(route.parentId !== undefined && route.parentId ===0 && route.path===parentName){
       parentPath = route.path
       return route
      }
      return []
  })    
 return {rt,parentPath}
}
