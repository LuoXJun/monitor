<template>
    <el-form
ref="baseFormRef" :model="modelValue" :label-width="labelWidth" :rules="rules" class="lxj-base-form-class"
        :label-position="labelPosition">
        <el-row>
            <template v-for="item in formItemList" :key="item.type">
                <el-col
v-bind="item.useLayout == false ? undefined : layout" :span="item.span"
                    v-if="item.isShow !== false">
                    <el-form-item
:label="item.label" :label-width="item.labelWidth" :prop="item.filed"
                        :rules="item.rules" :label-position="item.labelPosition">
                        <template #label="{ label }">
                            <div class="base-form-label">
                                <span>{{ label }}</span>
                                <el-tooltip
v-if="item.description" popper-class="base-form-tooltip"
                                    :trigger="item.description.trigger" :content="item.description.content"
                                    :placement="item.description.placement ?? 'bottom-start'">
                                    <svg
xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14"
                                        fill="none" style="margin-left: 2px; cursor: pointer" v-if="item.description">
                                        <path
                        ...
                        <template v-if="item.type == 'input' || item.type == 'password'">
                            <el-input
v-model="modelValue[item.filed]" clearable :disabled="item.disabled"
                                :type="item.type" :placeholder="item.placeholder" v-bind="item.input" />
                        </template>
                        <template v-else-if="item.type == 'select'">
                            <el-select
v-model="modelValue[item.filed]" clearable :placeholder="item.placeholder"
                                :disabled="item.disabled" v-bind="item.select">
                                <el-option
v-for="(option, index) in item.select?.options" :key="index"
                                    :label="option.label" :value="option.value" />
                            </el-select>
                        </template>
                        <template v-else-if="item.type == 'textarea'">
                            <el-input
type="textarea" v-model="modelValue[item.filed]" clearable
                                :placeholder="item.placeholder" :disabled="item.disabled" v-bind="item.textarea" />
                        </template>
                        <template v-else-if="item.type == 'date'">
                            <el-date-picker
v-model="modelValue[item.filed]" clearable :disabled="item.disabled"
                                type="date" :placeholder="item.placeholder ?? '请选择时间'"
                                value-format="YYYY-MM-DD HH:mm:ss" v-bind="item.date" />
                        </template>
                        <template v-else-if="item.type == 'daterange'">
                            <el-date-picker
v-model="modelValue[item.filed]" clearable :disabled="item.disabled"
                                value-format="YYYY-MM-DD HH:mm:ss" type="daterange" v-bind="item.date"
                                start-placeholder="开始时间" end-placeholder="结束时间" />
                        </template>
                        <template v-else-if="item.type == 'radio'">
                            <el-radio-group v-model="modelValue[item.filed]" :disabled="item.disabled">
                                <el-radio
v-for="(option, index) in item.radio?.options" :key="index"
                                    :label="option.value" :border="option.border">
                                    {{ option.label }}
                                </el-radio>
                            </el-radio-group>
                        </template>
...
v-for="(option, index) in item.radio?.options" :key="index"
                        ...
                        <template v-else-if="item.type == 'checkbox'">
                            <el-checkbox-group v-model="modelValue[item.filed]">
                                <el-checkbox
v-for="(option, index) in item.checkbox?.options" :key="index"
                                    :label="option.label" :border="option.border" />
                            </el-checkbox-group>
                        </template>
