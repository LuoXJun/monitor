  ...
  >
    <div class="dialog-content">
      ...
      <el-tabs v-model="activeName" @tab-click="handleTabClick">
        ...
        <el-tab-pane :label="PROCESS_TABS_MAP.clDetail.name" :name="PROCESS_TABS_MAP.clDetail.key">
          <div class="card-wrap">
            <el-card v-loading="processInstanceLoading" class="box-card">
              <WtDetail
                :sampleId="sampleId"
...
// 直接用检验中心的
import ProcessInstanceBpmnViewer from '@/views/taskManage/detail/ProcessInstanceBpmnViewer.vue'
// 直接用物探的
import ProcessInstanceTaskList from '@/views/wtTaskManage/detail/ProcessInstanceTaskList.vue'
...
import WtDetail from './WtDetail.vue'
...
const activeName = ref(PROCESS_TABS_MAP.clDetail.key)
...
const handleFillingAudit = async (task, pass) => {
  ...
  try {
    ...
    if (pass) {
      ...
      // await TaskApi.rejectTask(data)
      await SampleTaskApi.reject0Task(data)
      message.success('审批不通过成功')
      handleSuccess()
    }
  } finally {
...
const checkIsNeedNext = () => {
  ...
  nodeUserSelectTasks.value.forEach((item, index) => {
    console.log('item:', item, tasks.value[0].taskDefinitionKey)
    // 排查当前在哪一节点
...
const setTabsActive = () => {
  if (runningTasks.value?.length) {
    ...
    activeName.value = PROCESS_TABS_MAP.clDetail.key
...
.card-wrap {
  width: calc(1150px - var(--el-dialog-padding-primary) * 2 - 15px * 2 - 7px);
  margin: 0 auto;
}

.dialog-content {
  height: calc(100vh - var(--el-dialog-padding-primary) * 2 - 54px - 15px * 2);

  :deep(.el-tabs__content) {
    height: calc(100vh - var(--el-dialog-padding-primary) * 2 - 54px - 15px * 2 - 40px);
    overflow: auto;
  }
