<template>
    <div class="tableStatistics">
        <table v-if="currentData.list.length > 0">
            <tr>
...
const labels = [
    ...
    { name: '安装位置', mapping: 'buryLocation' }
];
...
const getData = async () => {
    ...
    currentData.dataTemplate.splice(
        ...
        0,
        {
            name: res.dataTemplate.name + '(最大值)',
            mapping: 'maxValue'
        },
        { name: '最大值日期', mapping: 'maxDate' },
        {
            name: res.dataTemplate.name + '(最小值)',
            mapping: 'minValue'
        },
        { name: '最小值日期', mapping: 'minDate' },
        {
            name: res.dataTemplate.name + '(当前值)',
            mapping: 'currentValue'
        },
        { name: '当前值日期', mapping: 'currentDate' }
    );
    // 处理特征值结构
    const obj = {};
