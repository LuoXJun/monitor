  ...
  <Dialog v-model="dialogVisible" :title="dialogTitle" width="600">
    ...
    >
      ...
      <el-form-item label="流程描述" prop="description">
        <el-input v-model="formData.description" clearable type="textarea" maxlength="500" />
      </el-form-item>
      <!-- 最新需求注释掉，想测通用工作流可以打开 -->
...
const formRules = reactive({
  ...
  key: [{ required: true, message: '参数键名不能为空', trigger: 'change' }],
  category: [{ required: true, message: '参数分类不能为空', trigger: 'change' }],
  value: [{ required: true, message: '参数键值不能为空', trigger: 'change' }],
  ...
  try {
    ...
    if (formType.value === 'create') {
      ...
      await ElMessageBox.alert(
        // '<div>1. 若为导入时，点击【修改流程】按钮，配置流程的分类、表单信息</div>' +
